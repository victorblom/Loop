    ...
    func ifNecessary(
            ...
            lastTempBasal.endDate > date
        {
            ...
            if matchesRate(lastTempBasal.unitsPerHour),
                ...
                return self //set temp to current Loop default instead of canceling
            }
        } else if matchesRate(scheduledBasalRate) {
...
private func targetGlucoseValue(percentEffectDuration: Double,
    ...
    // The inflection point in time beyond initial interval: before it we use minValue, after it we linearly blend from minValue to maxValue
...
extension Collection where Iterator.Element == GlucoseValue {
    ...
    ///   - initialThreshold: The glucose value below which bolus is not recommended
    ...
    private func insulinCorrection(
        ...
        for prediction in self {
            ...
            // If any predicted value is below the initial threshold, return immediately
    ...
    func recommendedTempBasal(
        ...
        // dm61: commented out maxBasalRate variable, since lines below have been commented out
        // var maxBasalRate = maxBasalRate

        // TODO: Allow `highBasalThreshold` to be a configurable setting
        // dm61: commented lines below to allow high temping below correction range
